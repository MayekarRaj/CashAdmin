...
import 'package:cash_admin/screens/payment_details_screen.dart';
...
Widget _uncollectedPaymentsListView(DateTime date) {
  return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
    ...
    builder: (context, snapshot) {
      ...
      logger.i(Timestamp.fromDate(date));
      logger.i(Timestamp.fromDate(date).toDate().add(const Duration(days: 1)));
      if (snapshot.hasError) {
        return Center(child: Text('Error: ${snapshot.error}'));
      }
